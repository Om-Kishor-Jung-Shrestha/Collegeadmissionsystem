...
async function sendMail(
  ...
  try {
    await transporter.sendMail({
      from: `"${platform}" <${process.env.SMTP_USER}>`,
      to,
      subject,
      html,
    });
    ...
    // Important:
    // Do not swallow the email error.
    // Let the central error middleware handle it.
    throw error;
  }
}
...
export const emailService = {
  ...
  async sendOrderConfirmation(opts: {
    ...
    const formattedDate =
      formatDate(opts.purchasedAt);

    const accessLink =
      `${origin}/courses/${opts.courseId}/learn`;

    await sendMail(
      opts.email,
      `🎉 You're enrolled in: ${opts.courseName}`,
      baseTemplate(`
        <!-- Hero -->
        <div
          style="
        ...
        <p
          ...
          "
        >
          Your payment was successful and you now have
  ...
  async sendAdminInvite(
    ...
    await sendMail(
      email,
      `You are invited to join ${platform} as Admin`,
      baseTemplate(`
        <h2
          style="
            color:#1f2937;
